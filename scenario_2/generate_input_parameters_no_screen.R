...
generate_input_parameters_no_screen <- function(n_samples,
                                        starting_age = starting_age) {
  
  # **ADAPTED**
  prop_asymp <- 0.42 #assumes 42% of all FN (i.e. undiagnosed) CD cases are asymtomatic
  # **ADAPTED**
  prop_symp <- 0.58
...
spec_POCT <- expit(spec_POCT)
  ...
  p_diag_ever <- 1/3  #lifetime probability of late diagnosis in undiagnosed
  ...
  #####Developing long-term consequence
  ...
  log_or_osteoporosis_GFD <- rnorm(n_samples, mean = log(1.43), 
                                   sd =  ((log(1.78) - log(1.15))/(2*1.96))) 
  # **ADAPTED**
  log_or_osteoporosis_noGFD <- rnorm(n_samples, mean = log(
    (((prop_asymp/(1-p_diag_ever))*(1+0.5*0.77)+((prop_symp-p_diag_ever)/(1-p_diag_ever)*1.77))+(prop_asymp*(1+0.5*0.77)+prop_symp*1.77))/2),
    sd = (((prop_asymp/(1-p_diag_ever))*0+((prop_symp-p_diag_ever)/(1-p_diag_ever)*((log(2.65) - log(1.18))/(2*1.96))))+(prop_asymp*0+prop_symp*((log(2.65) - log(1.18))/(2*1.96))))/2)
  ...
  log_rr_NHL_noGFD <- rnorm(n_samples, mean = log(
    (((prop_asymp/(1-p_diag_ever))*(1+0.5*3.7)+((prop_symp-p_diag_ever)/(1-p_diag_ever)*4.7))+(prop_asymp*(1+0.5*3.7)+prop_symp*4.7))/2),
    sd = (((prop_asymp/(1-p_diag_ever))*0+((prop_symp-p_diag_ever)/(1-p_diag_ever)*((log(7.3) - log(2.9))/(2*1.96))))+(prop_asymp*0+prop_symp*((log(7.3) - log(2.9))/(2*1.96))))/2)
  ...
  log_rr_GIC_noGFD <- rnorm(n_samples, mean = log(
    (((prop_asymp/(1-p_diag_ever))*(1+0.5*1.33)+((prop_symp-p_diag_ever)/(1-p_diag_ever)*2.33))+(prop_asymp*(1+0.5*1.33)+prop_symp*2.33))/2),
    sd = (((prop_asymp/(1-p_diag_ever))*0+((prop_symp-p_diag_ever)/(1-p_diag_ever)*((log(4.04) - log(1.35))/(2*1.96))))+(prop_asymp*0+prop_symp*((log(4.04) - log(1.35))/(2*1.96))))/2)
  ...
  # **ADAPTED**
  utility_undiagnosedCD_adults <- (((prop_asymp/(1-p_diag_ever))*((0.866+0.65)/2)+((prop_symp-p_diag_ever)/(1-p_diag_ever)*0.65))+(prop_asymp*((0.866+0.65)/2)+prop_symp*0.65))/2 # 0.65 is from NCV data
  utility_undiagnosedCD_se_adults <-  (prop_asymp/(1-p_diag_ever))*0+((prop_symp-p_diag_ever)/(1-p_diag_ever)*((0.75-0.55)/(2*1.96))) 
  ...
  # **ADAPTED**
  utility_undiagnosedCD_children <- (((prop_asymp/(1-p_diag_ever))*((0.958+0.59)/2)+(((prop_symp)-p_diag_ever)/(1-p_diag_ever)*0.59))+(prop_asymp*((0.958+0.59)/2)+prop_symp*0.59))/2 # 0.59 is from NCV data
  utility_undiagnosedCD_se_children <-  (prop_asymp/(1-p_diag_ever))*0+((prop_symp-p_diag_ever)/(1-p_diag_ever)*((0.69-0.49)/(2*1.96))) 
  ...
  # **ADAPTED**
  cost_diagnosis_children_ns <- (((prop_asymp/(1-p_diag_ever))*(cost_diag_asymp+biopsy*probability_biopsy_children)) +(((1-prop_asymp)-p_diag_ever)/(1-p_diag_ever)*cost_diagnosis_children_ns_tmp))  
  cost_diagnosis_adults_ns <- (((prop_asymp/(1-p_diag_ever))*(cost_diag_asymp+biopsy*probability_biopsy_adults)) +(((1-prop_asymp)-p_diag_ever)/(1-p_diag_ever)*cost_diagnosis_adults_ns_tmp)) 
  
  # **ADAPTED**
  cost_undiagnosedCD_adults_tmp <-  ((649.96 - poli_general)/10.72)  #649 = pre_costs from NCV. 10.72 = mean duration of symptoms in NCV. 
  cost_undiagnosedCD_adults <- (((prop_asymp/(1-p_diag_ever))*(cost_undiagnosedCD_adults_tmp*0.5)+((prop_symp-p_diag_ever)/(1-p_diag_ever)*cost_undiagnosedCD_adults_tmp))+(prop_asymp*(cost_undiagnosedCD_adults_tmp*0.5)+prop_symp*cost_undiagnosedCD_adults_tmp))/2
  ...
  cost_undiagnosedCD_children <-  (((prop_asymp/(1-p_diag_ever))*(cost_undiagnosedCD_children_tmp*0.5)+((prop_symp-p_diag_ever)/(1-p_diag_ever)*cost_undiagnosedCD_children_tmp))+(prop_asymp*(cost_undiagnosedCD_children_tmp*0.5)+prop_symp*cost_undiagnosedCD_children_tmp))/2 
 ...
 #productivity losses (friction cost method)
  # **ADAPTED**
  cost_abs_undiagnosed_fc <- (((prop_asymp/(1-p_diag_ever))*(1674.54*0.5)+((prop_symp-p_diag_ever)/(1-p_diag_ever)*1674.54))+(prop_asymp*(1674.54*0.5)+prop_symp*1674.54))/2
  cost_abs_undiagnosed_fc_se <- (((prop_asymp/(1-p_diag_ever))*0+((prop_symp-p_diag_ever)/(1-p_diag_ever)*76.83))+(prop_asymp*0+prop_symp*76.83))/2
