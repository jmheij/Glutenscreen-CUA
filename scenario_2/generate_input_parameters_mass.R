...
generate_input_parameters_mass <- function(n_samples,
...
spec_POCT <- expit(spec_POCT)
  ...
  # **ADAPTED**
  tp_subgroup <- 1*mean(sens_POCT)
  fn_subgroup <- 1-tp_subgroup
  subgroup_all <- mean(sens_POCT)
  
  # **ADAPTED**
  prop_asymp <- 0.42 #note that this quantity is prop_asymp among FN after screening 
  prop_symp <- 0.58
  
  # **ADAPTED**
  tp_symp <- prop_symp*mean(sens_POCT)
  tp_asymp <- prop_asymp*mean(sens_POCT)

  prop_children_diagnosis <- 0.40 #  based on Bergman et al 2022
  
  p_diag_ever <- (1/3)  #lifetime probability of  getting a late diagnosis
  p_diag_children <- prop_children_diagnosis*p_diag_ever  #probability of late diagnosis occurring in childhood.
  
  # **ADAPTED**
  p_diag_ever_tmp <- (1/3)*fn_subgroup #note that all of these will be symptomatic
  symp_fn <- prop_symp-(prop_symp*mean(sens_POCT)) #missed symptomatics
  asymp_fn <- prop_asymp-(prop_asymp*mean(sens_POCT)) #missed asymptomatics
  p_diag_ever_tmp2 <- symp_fn-p_diag_ever_tmp #symptomatics remaining undiagnosed
  
  probability_late_diagnosis_children <- 1 - (1-p_diag_children)^(1/17) 
  probability_late_diagnosis_adults <- 1 - ((1-p_diag_ever)/(1-p_diag_children))^(1/35)  
  ...
  # **ADAPTED**
  log_or_osteoporosis_GFD <- rnorm(n_samples, mean = log(
    (((tp_asymp/subgroup_all)*(1+0.5*0.43)+(tp_symp/subgroup_all)*1.43)+
       ((tp_asymp/(subgroup_all+p_diag_ever_tmp))*(1+0.5*0.43)+((tp_symp+p_diag_ever_tmp)/(subgroup_all+p_diag_ever_tmp))*1.43))/2),
    sd = (((tp_asymp/subgroup_all)*0+(tp_symp/subgroup_all)*((log(1.78) - log(1.15))/(2*1.96)))+
            ((tp_asymp/(subgroup_all+p_diag_ever_tmp))*0+((tp_symp+p_diag_ever_tmp)/(subgroup_all+p_diag_ever_tmp))*((log(1.78) - log(1.15))/(2*1.96))))/2) #Olmos 2008
  
  # **ADAPTED**
  log_or_osteoporosis_noGFD <- rnorm(n_samples, mean = log(
    (((asymp_fn/fn_subgroup)*(1+0.5*0.77)+(symp_fn/fn_subgroup)*1.77)+
       ((asymp_fn/(fn_subgroup-p_diag_ever_tmp))*(1+0.5*0.77)+(p_diag_ever_tmp2/(fn_subgroup-p_diag_ever_tmp))*1.77))/2),
    sd = (((asymp_fn/fn_subgroup)*0+(symp_fn/fn_subgroup)*((log(2.65) - log(1.18))/(2*1.96)))+
            ((asymp_fn/(fn_subgroup-p_diag_ever_tmp))*0+(p_diag_ever_tmp2/(fn_subgroup-p_diag_ever_tmp))*((log(2.65) - log(1.18))/(2*1.96))))/2)
  ...
  # **ADAPTED**
  log_rr_NHL_GFD <- rnorm(n_samples, mean = log(
    (((tp_asymp/subgroup_all)*(1+0.5*2.28)+(tp_symp/subgroup_all)*3.28)+
       ((tp_asymp/(subgroup_all+p_diag_ever_tmp))*(1+0.5*2.28)+((tp_symp+p_diag_ever_tmp)/(subgroup_all+p_diag_ever_tmp))*3.28))/2),
    sd = (((tp_asymp/subgroup_all)*0+(tp_symp/subgroup_all)*((log(6.28) - log(1.49))/(2*1.96)))+
            ((tp_asymp/(subgroup_all+p_diag_ever_tmp))*0+((tp_symp+p_diag_ever_tmp)/(subgroup_all+p_diag_ever_tmp))*((log(6.28) - log(1.49))/(2*1.96))))/2)
  
  # **ADAPTED**
  log_rr_NHL_noGFD <- rnorm(n_samples, mean = log(
    (((asymp_fn/fn_subgroup)*(1+0.5*3.7)+(symp_fn/fn_subgroup)*4.7)+
       ((asymp_fn/(fn_subgroup-p_diag_ever_tmp))*(1+0.5*3.7)+(p_diag_ever_tmp2/(fn_subgroup-p_diag_ever_tmp))*4.7))/2),
    sd = (((asymp_fn/fn_subgroup)*0+(symp_fn/fn_subgroup)*((log(7.3) - log(2.9))/(2*1.96)))+
            ((asymp_fn/(fn_subgroup-p_diag_ever_tmp))*0+(p_diag_ever_tmp2/(fn_subgroup-p_diag_ever_tmp))*((log(7.3) - log(2.9))/(2*1.96))))/2)
  ...
  # **ADAPTED**
  log_rr_GIC_GFD <- rnorm(n_samples, mean = log(
    (((tp_asymp/subgroup_all)*(1+0.5*0.32)+(tp_symp/subgroup_all)*1.32)+
       ((tp_asymp/(subgroup_all+p_diag_ever_tmp))*(1+0.5*0.32)+((tp_symp+p_diag_ever_tmp)/(subgroup_all+p_diag_ever_tmp))*1.32))/2),
    sd = (((tp_asymp/subgroup_all)*0+(tp_symp/subgroup_all)*((log(1.43) - log(1.22))/(2*1.96)))+
            ((tp_asymp/(subgroup_all+p_diag_ever_tmp))*0+((tp_symp+p_diag_ever_tmp)/(subgroup_all+p_diag_ever_tmp))*((log(1.43) - log(1.22))/(2*1.96))))/2)
  
  # **ADAPTED**
  log_rr_GIC_noGFD <- rnorm(n_samples, mean = log(
    (((asymp_fn/fn_subgroup)*(1+0.5*1.33)+(symp_fn/fn_subgroup)*2.33)+
       ((asymp_fn/(fn_subgroup-p_diag_ever_tmp))*(1+0.5*1.33)+(p_diag_ever_tmp2/(fn_subgroup-p_diag_ever_tmp))*2.33))/2),
    sd = (((asymp_fn/fn_subgroup)*0+(symp_fn/fn_subgroup)*((log(4.04) - log(1.35))/(2*1.96)))+
            ((asymp_fn/(fn_subgroup-p_diag_ever_tmp))*0+(p_diag_ever_tmp2/(fn_subgroup-p_diag_ever_tmp))*((log(4.04) - log(1.35))/(2*1.96))))/2)
  ...
  death_log_hazard_NHL <- exp(-2.6026)  # 2.6026 based on IKNL (indolent and aggresive NHL)
  ...
  # **ADAPTED**
  utility_GFD_adults <- (((tp_asymp/subgroup_all)*((0.866+0.84)/2)+(tp_symp/subgroup_all)*0.84)+
                           ((tp_asymp/(subgroup_all+p_diag_ever_tmp))*((0.866+0.84)/2)+((tp_symp+p_diag_ever_tmp)/(subgroup_all+p_diag_ever_tmp))*0.84))/2 # 0.83 is from NCV data
  utility_GFdse_adults <- (((tp_asymp/subgroup_all)*0+(tp_symp/subgroup_all)*((0.94-0.74)/(2*1.96)))+
                             ((tp_asymp/(subgroup_all+p_diag_ever_tmp))*0+((tp_symp+p_diag_ever_tmp)/(subgroup_all+p_diag_ever_tmp))*((0.94-0.74)/(2*1.96))))/2 # 0.001 is from NCV data
  ...
  # **ADAPTED**
  utility_GFD_children <-  (((tp_asymp/subgroup_all)*((0.958+0.9)/2)+(tp_symp/subgroup_all)*0.9)+
                              ((tp_asymp/(subgroup_all+p_diag_ever_tmp))*((0.958+0.9)/2)+((tp_symp+p_diag_ever_tmp)/(subgroup_all+p_diag_ever_tmp))*0.9))/2 # 0.9 is from NCV data
  utility_GFdse_children <-  (((tp_asymp/subgroup_all)*0+(tp_symp/subgroup_all)*((1-0.8)/(2*1.96)))+
                                ((tp_asymp/(subgroup_all+p_diag_ever_tmp))*0+((tp_symp+p_diag_ever_tmp)/(subgroup_all+p_diag_ever_tmp))*((1-0.8)/(2*1.96))))/2 # 0.01 is from NCV data
  ...
  # **ADAPTED**
  utility_undiagnosedCD_adults <- (((asymp_fn/fn_subgroup)*((0.866+0.65)/2)+(symp_fn/fn_subgroup)*0.65)+
                                     ((asymp_fn/(fn_subgroup-p_diag_ever_tmp))*((0.866+0.65)/2)+(p_diag_ever_tmp2/(fn_subgroup-p_diag_ever_tmp))*0.65))/2 # 0.65 is from NCV data
  utility_undiagnosedCD_se_adults <- (((asymp_fn/fn_subgroup)*0+(symp_fn/fn_subgroup)*((0.75-0.55)/(2*1.96)))+
                                        ((asymp_fn/(fn_subgroup-p_diag_ever_tmp))*0+(p_diag_ever_tmp2/(fn_subgroup-p_diag_ever_tmp))*((0.75-0.55)/(2*1.96))))/2 # 0.01 is from NCV data
  ...
  # **ADAPTED**
  utility_undiagnosedCD_children <- (((asymp_fn/fn_subgroup)*((0.958+0.59)/2)+(symp_fn/fn_subgroup)*0.59)+
                                       ((asymp_fn/(fn_subgroup-p_diag_ever_tmp))*((0.958+0.59)/2)+(p_diag_ever_tmp2/(fn_subgroup-p_diag_ever_tmp))*0.59))/2 # 0.59 is from NCV data  
  utility_undiagnosedCD_se_children <- (((asymp_fn/fn_subgroup)*0+(symp_fn/fn_subgroup)*((0.69-0.49)/(2*1.96)))+
                                          ((asymp_fn/(fn_subgroup-p_diag_ever_tmp))*0+(p_diag_ever_tmp2/(fn_subgroup-p_diag_ever_tmp))*((0.69-0.49)/(2*1.96))))/2 # 0.02 is from NCV data
  ...
  # cost_gfp <- 1506.02  # from NCV data
  # cost_gfp_se <- 16.93  # from NCV data
  cost_gfp <- (((tp_asymp/subgroup_all)*(1506.02*0.5)+(tp_symp/subgroup_all)*1506.02)+
                          ((tp_asymp/(subgroup_all+p_diag_ever_tmp))*(1506.02*0.5)+((tp_symp+p_diag_ever_tmp)/(subgroup_all+p_diag_ever_tmp))*1506.02))/2 # 
  cost_gfp_se <- (((tp_asymp/subgroup_all)*0+(tp_symp/subgroup_all)*16.93)+
                             ((tp_asymp/(subgroup_all+p_diag_ever_tmp))*0+((tp_symp+p_diag_ever_tmp)/(subgroup_all+p_diag_ever_tmp))*16.93))/2 # 
  ...
  # **ADAPTED**
  cost_diagnosis_children_ns <-  (((asymp_fn/fn_subgroup)*(cost_diag_asymp+biopsy*mean(probability_biopsy_children))+(symp_fn/fn_subgroup)*cost_diagnosis_children_ns_tmp)+
                                    ((asymp_fn/(fn_subgroup-p_diag_ever_tmp))*(cost_diag_asymp+biopsy*mean(probability_biopsy_children))+(p_diag_ever_tmp2/(fn_subgroup-p_diag_ever_tmp))*cost_diagnosis_children_ns_tmp))/2 
  
  # **ADAPTED**
  cost_diagnosis_adults_ns <- (((asymp_fn/fn_subgroup)*(cost_diag_asymp+biopsy*mean(probability_biopsy_adults))+(symp_fn/fn_subgroup)*cost_diagnosis_adults_ns_tmp)+
                                 ((asymp_fn/(fn_subgroup-p_diag_ever_tmp))*(cost_diag_asymp+biopsy*mean(probability_biopsy_adults))+(p_diag_ever_tmp2/(fn_subgroup-p_diag_ever_tmp))*cost_diagnosis_adults_ns_tmp))/2 
  
  # **ADAPTED**
  cost_undiagnosedCD_adults_tmp <-  ((649.96 - poli_general)/10.72) #649 = pre_costs from NCV. 10.72 = mean duration of symptoms in NCV. 
  cost_undiagnosedCD_adults <- (((asymp_fn/fn_subgroup)*(cost_undiagnosedCD_adults_tmp*0.5)+(symp_fn/fn_subgroup)*cost_undiagnosedCD_adults_tmp)+
                                                             ((asymp_fn/(fn_subgroup-p_diag_ever_tmp))*(cost_undiagnosedCD_adults_tmp*0.5)+(p_diag_ever_tmp2/(fn_subgroup-p_diag_ever_tmp))*cost_undiagnosedCD_adults_tmp))/2 
  ...
  cost_undiagnosedCD_children <- (((asymp_fn/fn_subgroup)*(cost_undiagnosedCD_children_tmp*0.5)+(symp_fn/fn_subgroup)*cost_undiagnosedCD_children_tmp)+
                                                               ((asymp_fn/(fn_subgroup-p_diag_ever_tmp))*(cost_undiagnosedCD_children_tmp*0.5)+(p_diag_ever_tmp2/(fn_subgroup-p_diag_ever_tmp))*cost_undiagnosedCD_children_tmp))/2 
  ...
  cost_CDGFD_adults <- (((tp_asymp/subgroup_all)*(172.85*0.5)+(tp_symp/subgroup_all)*172.85)+
                           ((tp_asymp/(subgroup_all+p_diag_ever_tmp))*(172.85*0.5)+((tp_symp+p_diag_ever_tmp)/(subgroup_all+p_diag_ever_tmp))*172.85))/2 # 
  cost_CDGFD_se_adults <- (((tp_asymp/subgroup_all)*0+(tp_symp/subgroup_all)*(172.85*0.1))+
                             ((tp_asymp/(subgroup_all+p_diag_ever_tmp))*0+((tp_symp+p_diag_ever_tmp)/(subgroup_all+p_diag_ever_tmp))*(172.85*0.1)))/2 # 
  ...
  cost_CDGFD_children <-  (((tp_asymp/subgroup_all)*(205.65*0.5)+(tp_symp/subgroup_all)*205.65)+
                              ((tp_asymp/(subgroup_all+p_diag_ever_tmp))*(205.65*0.5)+((tp_symp+p_diag_ever_tmp)/(subgroup_all+p_diag_ever_tmp))*205.65))/2 # 
  cost_CDGFD_se_children <-  (((tp_asymp/subgroup_all)*0+(tp_symp/subgroup_all)*(205.65*0.1))+
                                ((tp_asymp/(subgroup_all+p_diag_ever_tmp))*0+((tp_symp+p_diag_ever_tmp)/(subgroup_all+p_diag_ever_tmp))*(205.65*0.1)))/2 # 
 ...
 #######################################################################################
  ...
  # **ADAPTED**
  cost_abs_undiagnosed_fc <- (((asymp_fn/fn_subgroup)*(1674.57*0.5)+(symp_fn/fn_subgroup)*1674.57)+
                                                           ((asymp_fn/(fn_subgroup-p_diag_ever_tmp))*(1674.57*0.5)+(p_diag_ever_tmp2/(fn_subgroup-p_diag_ever_tmp))*1674.57))/2
  cost_abs_undiagnosed_fc_se <- (((asymp_fn/fn_subgroup)*0+(symp_fn/fn_subgroup)*76.83)+
                                                              ((asymp_fn/(fn_subgroup-p_diag_ever_tmp))*0+(p_diag_ever_tmp2/(fn_subgroup-p_diag_ever_tmp))*76.83))/2
...
cost_abs_undiagnosed_fc_beta <- (cost_abs_undiagnosed_fc_se^2)/cost_abs_undiagnosed_fc  
 cost_abs_undiagnosed_fc <- rgamma(n=n_samples, shape = cost_abs_undiagnosed_fc_alpha, scale = cost_abs_undiagnosed_fc_beta)
  ...
  # **ADAPTED**
 cost_abs_diagnosed_fc <-(((tp_asymp/subgroup_all)*(332.12*0.5)+(tp_symp/subgroup_all)*332.12)+
                                ((tp_asymp/(subgroup_all+p_diag_ever_tmp))*(332.12*0.5)+((tp_symp+p_diag_ever_tmp)/(subgroup_all+p_diag_ever_tmp))*332.12))/2
  cost_abs_diagnosed_fc_se <- (((tp_asymp/subgroup_all)*0+(tp_symp/subgroup_all)*33.09)+
                                                            ((tp_asymp/(subgroup_all+p_diag_ever_tmp))*0+((tp_symp+p_diag_ever_tmp)/(subgroup_all+p_diag_ever_tmp))*33.09))/2
