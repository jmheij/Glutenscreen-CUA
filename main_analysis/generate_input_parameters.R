...
###############################################################################
#This R script generates a data frame with 1000 rows containing the point estimates
#of the model parameters that will be used in each PSA iteration.
#For more information about the sources and/or calculations, refer to the manuscript Methods and Supplement.
###############################################################################
...
generate_input_parameters <- function(n_samples,
  ...
  p_diag_ever <- 1/3  #assumed probability of ever getting a diagnosis if entering the model as undiagnosed.
  p_diag_children <- prop_children_diagnosis*p_diag_ever 
  
  
  probability_late_diagnosis_children <- 1 - (1-p_diag_children)^(1/17) 
  probability_late_diagnosis_adults <- 1 - ((1-p_diag_ever)/(1-p_diag_children))^(1/35)  #the 35 and the 17 are the result of iterative calibration in order to achieve the assumption that 1 in 3 undiagnosed people reach the diagnosed state throughout the model under current practice (checked using the trace for no screening).  
  
  
  
  # Use the appropriate prevalence of long-term consequences
  prevalence = as.data.frame(readxl::read_excel(path = "prevalence/adapted from CPRD prevalence.xlsx", sheet = "mixed")) 
  
  #Initial cohort at diagnosis for the age of our cohort
  ...
  # Prevalence of osteoporosis, NHL, and GIC are combined with incidence rates to model prevalence changing with age
  ...
  #####Developing long-term consequences
  ...
  death_log_hazard_NHL <- exp(-2.6026)  # based on IKNL (indolent and aggressive NHL)
  ...
  utility_undiagnosedCD_adults <-  0.65 # 0.65 is from NCV data
  utility_undiagnosedCD_se_adults <- (0.75-0.55)/(2*1.96)  # 0.01 
  ...
  utility_undiagnosedCD_children <- 0.59 # 0.59 is from NCV data
  utility_undiagnosedCD_se_children <- (0.69-0.49)/(2*1.96)  # 0.02 
  ...
  cost_osteoporosis <- (110700000/(29.2*17340))*1.0398 # 110,700,000 euros in 2019, 506328 (i.e. 29.2 per 1000 * 17340) people with OSTP in 2019 (https://WWW.vzinfo.nl/osteoporose/zorguitgaven and https://www.nivel.nl/nl/nivel-zorgregistraties-eerste-lijn/jaarcijfers-aandoeningen-huisartsenregistraties )
  ...
  cost_gfp <- 1506.02  # from NCV data, year 2021
...
cost_diagnosis_s <- test_battery + (biopsy*(5/61)) + poli_pediatric + dietician + phone_doc # 5/61 = prop receiving biopsy in GLUTENSCREEN

poli_general <- 101 #for use later 2021 eur, from costing manual, gewogen gemidelde
...
cost_diagnosis_children_ns <- test_battery + poli_pediatric + (probability_biopsy_children*biopsy) #costs of clinical diagnosis if delayed diagnosis
cost_diagnosis_adults_ns <- test_battery + poli_general + (probability_biopsy_adults*biopsy)  #costs of clinical diagnosis if delayed diagnosis

cost_undiagnosedCD_adults <-  ((649.96 - poli_general)/10.72)  #649 = pre_costs from NCV. 10.72 = mean duration of symptoms in NCV. 
...
cost_undiagnosedCD_children <- ((680.8 - mean(cost_diagnosis_children_ns))/1.96) # 680.8 = costs from NSCK. 1.96 = mean duration of symptoms in NCV.
...
#Absence from work
cost_abs_undiagnosed_fc <- 1674.54 #2021 eur . NCV data
cost_abs_undiagnosed_fc_se <- 76.83
...
input_parameters <- data.frame(probability_late_diagnosis_children, probability_late_diagnosis_adults, 
                               ...
                               utility_GFD_children, utility_GFD_adults, utility_undiagnosedCD_children, utility_undiagnosedCD_adults,  disutility_osteoporosis, disutility_NHL, disutility_GIC,
