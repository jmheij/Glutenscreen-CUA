...
spec_POCT <- expit(spec_POCT)
  ...
  log_or_osteoporosis_noGFD <- rnorm(n_samples, mean = log(
    (((prop_asymp/(1-p_diag_ever))*1+((prop_symp-p_diag_ever)/(1-p_diag_ever)*1.77))+(prop_asymp*1+prop_symp*1.77))/2),
  ...
  log_rr_NHL_noGFD <- rnorm(n_samples, mean = log(
    (((prop_asymp/(1-p_diag_ever))*1+((prop_symp-p_diag_ever)/(1-p_diag_ever)*4.7))+(prop_asymp*1+prop_symp*4.7))/2),
  ...
  log_rr_GIC_noGFD <- rnorm(n_samples, mean = log(
    (((prop_asymp/(1-p_diag_ever))*1+((prop_symp-p_diag_ever)/(1-p_diag_ever)*2.33))+(prop_asymp*1+prop_symp*2.33))/2),
  ...
  # **ADAPTED**
  utility_undiagnosedCD_adults <- (((prop_asymp/(1-p_diag_ever))*0.866+((prop_symp-p_diag_ever)/(1-p_diag_ever)*0.65))+(prop_asymp*0.866+prop_symp*0.65))/2 # 0.65 is from NCV data
  utility_undiagnosedCD_se_adults <-  (prop_asymp/(1-p_diag_ever))*(0.17/sqrt(953))+((prop_symp-p_diag_ever)/(1-p_diag_ever)*0.01) 
  ...
  # **ADAPTED**
  utility_undiagnosedCD_children <- (((prop_asymp/(1-p_diag_ever))*0.958+(((prop_symp)-p_diag_ever)/(1-p_diag_ever)*0.59))+(prop_asymp*0.958+prop_symp*0.59))/2 # 0.59 is from NCV data
  utility_undiagnosedCD_se_children <-  (prop_asymp/(1-p_diag_ever))*(0.07/sqrt(26))+((prop_symp-p_diag_ever)/(1-p_diag_ever)*0.02) 
  ...
  cost_undiagnosedCD_adults <- (((prop_asymp/(1-p_diag_ever))*0+((prop_symp-p_diag_ever)/(1-p_diag_ever)*cost_undiagnosedCD_adults_tmp))+(prop_asymp*0+prop_symp*cost_undiagnosedCD_adults_tmp))/2
  ...
  cost_undiagnosedCD_children <-  (((prop_asymp/(1-p_diag_ever))*0+((prop_symp-p_diag_ever)/(1-p_diag_ever)*cost_undiagnosedCD_children_tmp))+(prop_asymp*0+prop_symp*cost_undiagnosedCD_children_tmp))/2 
 ...
 #productivity losses (friction cost method)
  # **ADAPTED**
  cost_abs_undiagnosed_fc <- (((prop_asymp/(1-p_diag_ever))*0+((prop_symp-p_diag_ever)/(1-p_diag_ever)*1674.54))+(prop_asymp*0+prop_symp*1674.54))/2
