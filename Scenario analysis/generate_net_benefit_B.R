...
# generate_net_benefit function used for the main analysis.
...
generate_net_benefit_B <- function(input_parameters, 
                                   ...
                                   lambda)
{
  ...
  # True positives etc due to symptom questionnaire are the same across POCT strategies
  ...
  biopsy_disutility_applied <- (disutility_biopsy_screen * ((tp[,] + fp[,])*5/61))/pre_test_probability_overall # only about 8% of those who tested positive on the POCT will receive a biopsy
          ...
          #**ADAPTED**
          #call correct values for this output from previous calls to the model
          biopsy_disutility_applied[,'No screening'] <- output_no_screening$disutility_biopsy
          biopsy_disutility_applied[,'0.9999 0 POCT'] <- output_0.9999_0_POCT$disutility_biopsy
  ...
  # for (i in c(1:3)) {
  #   writexl::write_xlsx(cv_tests[i], paste0("new_coh_vec_", i, ".xlsx"))
  # }
  ...
  total_costs <- t(total_costs)  #add Questionnaire cost pre CD case here (below)
  total_costs[2:n_tests,] <- total_costs[2:n_tests,]+(input_parameters$cost_quest[1]/mean(pre_test_probability_overall)) 

            #**ADAPTED**
             #call correct values for this output from previous calls to the model
            total_costs['No screening',] <- output_no_screening$total_costs
            total_costs['0.9999 0 POCT',] <- output_0.9999_0_POCT$total_costs
  
  total_qalys <- t(total_qalys)
  
            #**ADAPTED**
             #call correct values for this output from previous calls to the model
            total_qalys['No screening',] <- output_no_screening$total_qalys
            total_qalys['0.9999 0 POCT',] <- output_0.9999_0_POCT$total_qalys
  ...
  output$cycle_costs <- colMeans(cycle_costs)
            ...
            #**ADAPTED**
            #call correct values for this output from previous calls to the model
            output$cycle_costs['No screening'] <- output_no_screening$cycle_costs
            output$cycle_costs['0.9999 0 POCT'] <- output_0.9999_0_POCT$cycle_costs
  ...
  output$cycle_qalys <- colMeans(cycle_qalys)
             ...
             #**ADAPTED**
             #call correct values for this output from previous calls to the model
            output$cycle_qalys['No screening'] <- output_no_screening$cycle_qalys
            output$cycle_qalys['0.9999 0 POCT'] <- output_0.9999_0_POCT$cycle_qalys
