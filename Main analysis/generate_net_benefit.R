                                 ...
                                 lambda)
{
  ...
  ##for reference 
  unscaled_table <- data.frame(colMeans(tp), colMeans(fn), colMeans(tn), colMeans(fp))
  ...
  #for Markov traces:
  cv_tests <- as.data.frame(cohort_vectors_df)
  cv_tests <- split(as.data.frame(cv_tests), cv_tests$test)

  #collect traces in jpegs
  # for (t in 1:n_tests) {
  #   mtrace_temp <- ggplot(cv_tests[[t]], aes(x = cycle))
  # 
  #   stat_layers <- lapply(state_names, function(state) {
  #     stat_summary(aes(y = .data[[state]], color = state), fun = "mean", geom = "line", size = 1)
  #   })
  # 
  #   mtrace_temp <- mtrace_temp + stat_layers
  # 
  #   mtrace_temp <- mtrace_temp +
  #     labs(title = paste0("Markov cohort trace over cycles. Strategy = ", t_names[t]), x = "Cycle", y = "Proportion in state") +
  #     scale_x_continuous(breaks = seq(1, 97, 5)) +
  #     scale_y_continuous(breaks = seq(0, 1, 0.2)) +
  #     ylim(0, 1) +
  #     guides(color = guide_legend(title = ""))
  # 
  #   jpeg(file = paste0("Markov_traces/Mtrace_", t_names[t], ".jpeg"), quality = 100, height = 15, width = 30, units = "cm", res = 300)
  #   print(mtrace_temp)
  #   dev.off()
  # }
  ...
  total_costs <- t(total_costs)  #add Questionnaire cost per CD case only for case-finding strategies (below) Note strategies 1 and 2 dont have this cost.
  total_costs[3:n_tests,] <- total_costs[3:n_tests,]+(input_parameters$cost_quest[1]/mean(pre_test_probability_overall)) 
...
######################################
output$all_ICER <- array(dim=c(n_tests,n_samples), dimnames=list(t_names,NULL))
  
  for (i_sample in 1:n_samples) {
    for (i_test in 1:n_tests) {
      output$all_ICER[i_test, ] <- output$all_incremental_costs[i_test, ]/output$all_incremental_effects[i_test, ]
    }}
  
  output$ICER_lci <-   output$ICER_uci <- array(dim=c(n_tests,n_samples),  
                                              dimnames=list(t_names,NULL))
  
  for (i_sample in 1:n_samples) {
    for (i_test in 1:n_tests) {
      output$ICER_lci[i_test, i_sample] <- quantile(output$all_ICER[i_test,], 0.025, na.rm = TRUE)
      output$ICER_uci[i_test, i_sample] <- quantile(output$all_ICER[i_test,], 0.975, na.rm = TRUE)
      
    }
  }
  
  output$ICER_lci <- rowMeans(output$ICER_lci)
  output$ICER_uci <- rowMeans(output$ICER_uci)
  ####################################
  ...
  output$unscaled_table <- unscaled_table
