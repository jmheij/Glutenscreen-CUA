...
generate_transition_matrices <- function(input_parameters) {
  ...
  transition_matrices <- array(dim = c(n_samples, n_cycles, n_states, n_states),  # now n_states=9 
  ...
  for (i_cycle_tmp in 1:n_cycles) {
    ...
    #---ppt example---- 5. repeat exercise for new health state
    #from state to GIC
  ...
  # it reduces all other transitions (e.g., the % who do not die of NHL are divided among remaining probabilities)
