...
#The R Project folders for the sensitivity analyses (i.e. folders 'Scenario analysis - Glutenscreen CEA model' and 'Main analysis HC - Glutenscreen CEA model')
#contain modified versions of the R scripts included in this R project.
#######################################################################################
...
# Note: the main analysis compares two testing strategies out of the table below: "0.9999 0" and "0.58 0.63"
# rows 3-27 in the 'combinations' data frame below are further (secondary) analyses (some of which are mentioned in the manuscript Discussion).
...
#Cost breakdown table (only breaks down test costs, diagnosis costs, and cycle costs)
  #Note: for the cost breakdown presented in the manuscirpt (i.e. Talbes 1 and 2), 
  # the model needs to be re-run several times, once for each cost category, by ommitting 
  # other costs in the generate_state_costs function and/or by setting costs to 0 in the 
  # generate_input_parameters function. 
...
################################################################################################
# The below code is relevant only for the further analyses looking at strategies 4-28 in t_names
